...
class _QuizScreenState extends State<QuizScreen> {
      ...
      builder: (context) => PremiumDialog(
        ...
        content: Column(
          ...
          children: [
            Text(
              ...
              style: AppTextStyles.body(context).copyWith(color: Colors.white70, fontSize: 18),
        ...
        actions: [
          TextButton(
            ...
            child: Text(isArabic ? "إعادة" : "Retry", style: AppTextStyles.button(context).copyWith(color: Colors.white60)),
          ),
          ElevatedButton(
            onPressed: () {
              Navigator.pop(context); // Close Dialog
            ...
            child: Text(l10n.done, style: AppTextStyles.button(context)),
          ),
        ],
      ),
    );
  ...
  Widget build(BuildContext context) {
    ...
    if (_questions.isEmpty) return Scaffold(backgroundColor: AppColors.darkBackground, body: Center(child: Text(isArabic ? "لا توجد أسئلة" : "No questions", style: AppTextStyles.body(context).copyWith(color: Colors.white))));
    ...
    return Padding(
      ...
      child: Column(children: [
        Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
          Text(isArabic ? "السؤال ${_currentIndex + 1} من ${_questions.length}" : "Question ${_currentIndex + 1} of ${_questions.length}", style: AppTextStyles.sectionTitle(context).copyWith(color: AppColors.neutralMedium, fontSize: 13)),
          Text("${((_currentIndex + 1) / _questions.length * 100).round()}%", style: AppTextStyles.sectionTitle(context).copyWith(fontSize: 13)),
        ]),
    ...
    return Dialog(
      ...
      child: Container(
        ...
        child: Column(mainAxisSize: MainAxisSize.min, children: [
          ...
          Text(isPass ? l10n.congrats : "KEEP LEARNING!", style: AppTextStyles.cardTitle(context).copyWith(fontSize: 22, fontWeight: FontWeight.w900, color: Colors.white), textAlign: TextAlign.center),
          const SizedBox(height: 12),
          Text("You scored $score out of $total", style: AppTextStyles.body(context).copyWith(color: Colors.white.withOpacity(0.6), fontSize: 16), textAlign: TextAlign.center),
          const SizedBox(height: 32),
          Row(children: [
            Expanded(child: OutlinedButton(onPressed: onRetry, style: OutlinedButton.styleFrom(foregroundColor: AppColors.primaryGold, side: const BorderSide(color: AppColors.primaryGold), padding: const EdgeInsets.symmetric(vertical: 14), shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(12))), child: Text("RETRY", style: AppTextStyles.button(context).copyWith(color: AppColors.primaryGold)))),
            const SizedBox(width: 12),
            Expanded(child: ElevatedButton(onPressed: onFinish, style: ElevatedButton.styleFrom(backgroundColor: AppColors.primaryGold, foregroundColor: AppColors.darkInk, padding: const EdgeInsets.symmetric(vertical: 14), shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(12)), elevation: 0), child: Text("DONE", style: AppTextStyles.button(context)))),
          ]),
        ]),
      ),
    );
